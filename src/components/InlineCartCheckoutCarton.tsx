...
const InlineCartCheckoutCarton = () => {
  ...
  const handleCheckout = () => {
    ...
    toast({
      title: "Order placed!",
      description: "We'll contact you shortly to confirm your carton order."
    });
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('fireworksCheckoutDataCarton');
    }
  };
  ...
  const handleWhatsAppOrder = () => {
    ...
    const orderDetails = items.map(item => 
      `${item.name} (${item.type}) x${item.quantity} boxes = RM ${(item.price * item.quantity).toFixed(2)}`
    ).join('\n');
    ...
    return (
      <div className="bg-white border-4 border-yellow-400 rounded-2xl p-8 shadow-lg">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-black mb-4">📦 Your Carton Cart</h2>
          <p className="text-gray-600 mb-6">Add some carton boxes to see them here!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border-4 border-yellow-400 rounded-2xl p-6 shadow-lg">
      <h2 className="text-3xl font-bold text-black mb-6 text-center">📦 Complete Your Carton Order</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold text-black">
              Carton Items ({getTotalItems()} boxes)
            </h3>
            <Button
              onClick={clearCart}
              variant="outline"
              ...
              <div key={item.id} className="border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{item.type} - Per Box</p>
                    <p className="font-bold text-black text-sm">RM {item.price}/box</p>
                  </div>
                ...
                <div className="text-right mt-2">
                  <p className="font-bold text-black text-sm">
                    RM {(item.price * item.quantity).toFixed(2)} ({item.quantity} boxes)
                  </p>
                </div>
              </div>
            ))}
        ...
        <div>
          <div className="bg-gray-50 border-2 border-black rounded-lg p-6">
            <h3 className="text-xl font-bold text-black mb-4">Carton Order Summary</h3>
            
            <div className="mb-6">
              <div className="flex justify-between items-center py-2">
                <span>Total Boxes:</span>
                <span className="font-semibold">{getTotalItems()} boxes</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span>Subtotal:</span>
            ...
            <p className="text-xs text-gray-500 mt-4 text-center">
              * Required fields: Name, Phone, Address, Postcode, State, District. Carton orders are sold by boxes for wholesale quantities.
            </p>
          </div>
        </div>
      </div>
...
export default InlineCartCheckoutCarton;